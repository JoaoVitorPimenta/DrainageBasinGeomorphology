...
def EAVprocessing(demArray,noData,gt,proj,cols,rows,basin,feedback):
    ...
    cumulativeAreasList = cumulativeAreas[:-1]
    cumulativeVolumesList = cumulativeVolumes[:-1]
    return elevations, cumulativeAreasList, cumulativeVolumesList, validMask

def calcCuttedDEM(arrayInsideBasin, elevation, height, area, volume, folderRaster, folderVector, noDataValue, proj, gt, demLayer, openRasters, openVectors, basin, idx, step, feedback):
    feedback.setProgressText('Basin id '+str(basin.id())+' processing raster starting...')

    elevationsCuttedPixels = np.where(arrayInsideBasin >= elevation, elevation, arrayInsideBasin)

    if np.isnan(elevationsCuttedPixels).all():
        feedback.pushWarning('There are no DEM pixels within basin id '+str(basin.id())+' above respective elevation, so it is not possible to generate the output layers for this basin.')
        return

    mask = ~np.isnan(elevationsCuttedPixels)
    rows, cols = np.where(mask)

    rowMin, rowMax = rows.min(), rows.max()
    colMin, colMax = cols.min(), cols.max()
    croppedDepthArray = elevationsCuttedPixels[rowMin:rowMax+1, colMin:colMax+1]
    ...
    totalCuttedRaster = driver.Create(pathRaster, columns, lines, 1, gdal.GDT_Float32)

    totalCuttedRaster.SetGeoTransform(newGt)
    totalCuttedRaster.SetProjection(projecao)

    totalCuttedBand = totalCuttedRaster.GetRasterBand(1)
    totalCuttedBand.WriteArray(croppedDepthArray)

    totalCuttedBand.SetNoDataValue(noDataValue)
    ...
    cuttedRaster = driver.Create("MEM", columns, lines, 1, gdal.GDT_Float32)

    cuttedRaster.SetGeoTransform(newGt)
    cuttedRaster.SetProjection(projecao)

    cuttedBand = cuttedRaster.GetRasterBand(1)
    cuttedBand.WriteArray(croppedFilledArray)

    cuttedBand.SetNoDataValue(noDataValue)
    ...
    gdal.Polygonize(cuttedBand, cuttedBand, layer, 0, [], callback=None)
    ...
    layer = None
    mergedLayer = None
    totalCuttedRaster = None
    cuttedRaster = None
    cuttedBand = None
    totalCuttedBand = None
    rasterOriginal = None
...
def calcCuttedForParameter(drainageBasinLayer,demLayer,parameter,parameterValue,pathRaster,pathVector,noDataValue,openRasters,openVectors,feedback):
    ...
    for idx, basin in enumerate(drainageBasinLayer.getFeatures()):
        ...
        if len(elevations) == 0:
            feedback.pushWarning('There is no valid raster data in the basin of id '+str(basin.id())+' and therefore it is not possible to calculate the cutted DEM of ​​this basin.')
            return
        ...
        if parameter == HEIGHT_PARAMETER:
            ...
            calcCuttedDEM(arrayInsideBasin, respectiveElevation, respectiveHeight, respectiveArea, respectiveVolume, pathRaster, pathVector, noDataValue, proj, gt, demLayer, openRasters, openVectors, basin, idx, step, feedback)
