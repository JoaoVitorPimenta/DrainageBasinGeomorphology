...
def calcInundationArea(arrayInsideBasin, waterElevation, waterHeight, waterArea, waterVolume, folderRaster, folderVector, noDataValue, proj, gt, demLayer, negativeDepth, openRasters, openVectors, basin, idx, step, feedback):
    ...
    if negativeDepth is True:
        depthPixels = elevationsInundatedPixels - waterElevation
    else:
        depthPixels = abs(elevationsInundatedPixels - waterElevation)

    mask = ~np.isnan(depthPixels)
    rows, cols = np.where(mask)

    rowMin, rowMax = rows.min(), rows.max()
    colMin, colMax = cols.min(), cols.max()
    croppedDepthArray = depthPixels[rowMin:rowMax+1, colMin:colMax+1]
    ...
    depthRaster.SetProjection(projecao)

    depthBand = depthRaster.GetRasterBand(1)
    depthBand.WriteArray(croppedDepthArray)
    ...
    inundatedRaster = driver.Create("MEM", columns, lines, 1, gdal.GDT_Float32)

    inundatedRaster.SetGeoTransform(newGt)
    inundatedRaster.SetProjection(projecao)

    inundatedBand = inundatedRaster.GetRasterBand(1)
    inundatedBand.WriteArray(croppedInundatedArray)
    ...
    gdal.Polygonize(inundatedBand, inundatedBand, layer, 0, [], callback=None)
    ...
    feature.SetField("Height", waterHeight)
    feature.SetField("Elevation", waterElevation)
    feature.SetField("Area", waterArea)
    feature.SetField("Volume", waterVolume)
    mergedLayer.SetFeature(feature)

    inundatedBand.FlushCache()
    depthBand.FlushCache()
    layer.SyncToDisk()
...
def calcEAVForParameter(drainageBasinLayer,demLayer,parameter,parameterValue,pathRaster,pathVector,noDataValue,negativeDepth,openRasters,openVectors,feedback):
    ...
    for idx, basin in enumerate(drainageBasinLayer.getFeatures()):
        ...
        if len(elevations) == 0:
            feedback.pushWarning('There is no valid raster data in the basin of id '+str(basin.id())+' and therefore it is not possible to calculate the inundated area of ​​this basin.')
            return
        ...
        if parameter == HEIGHT_PARAMETER:
            ...
            waterElevation = parameterValue+elevations[0]
            waterHeight = parameterValue
            waterArea = heightAreaInterpolation
            waterVolume = heightVolInterpolation

            calcInundationArea(arrayInsideBasin, waterElevation, waterHeight, waterArea, waterVolume, pathRaster, pathVector, noDataValue, proj, gt, demLayer, negativeDepth, openRasters, openVectors, basin, idx, step, feedback)
        ...
        if parameter == ELEVATION_PARAMETER:
            ...
            waterElevation = parameterValue
            waterHeight = waterElevation - elevations[0]
            waterArea = elevationAreaInterpolation
            waterVolume = elevationVolInterpolation
        ...
        if parameter == AREA_PARAMETER:
            ...
            waterElevation = areaElevationInterpolation
            waterHeight = waterElevation - elevations[0]
            waterArea = parameterValue
            waterVolume = areaVolInterpolation
        ...
        if parameter == VOLUME_PARAMETER:
            ...
            waterElevation = volElevationInterpolation
            waterHeight = waterElevation - elevations[0]
            waterArea = volAreaInterpolation
            waterVolume = parameterValue
